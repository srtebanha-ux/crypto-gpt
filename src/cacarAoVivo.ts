...
// Gerenciador de Nonce Atômico Local para evitar conflitos em picos de alta frequência
...
public async sync(): Promise<number> {
    try {
        ...
        log.warn('Falha ao sincronizar nonce com a rede.', { erro: (e as Error).message });
    }
...
async function umaChamada(metodo: string, params: unknown[]): Promise {
...
async function chamar(metodo: string, params: unknown[], tentativas = 4): Promise {
let espera = 1000;
for (let i = 0; ; i += 1) {
try {
return await umaChamada(metodo, params);
} catch (e) {
...
if (ENVIAR) {
    ...
    log.info('Carteira carregada com Nonce Manager ativado.', { endereco: donoCarteira });
}
    ...
    try {
        ...
        for (const alvo of alvos) {
            for (const contrato of CONTRATOS_ATIVOS) {
                ...
                // Obtenção síncrona segura do próximo nonce local
                const nonceSeguro = await nonceManager.getNextNonce();
